...
"#;
    ...
    // 保存示例代码到文件
    let example_path = PathBuf::from("example_calculator.java");
    if let Err(e) = fs::write(&example_path, java_code) {
        eprintln!("Failed to write example file: {}", e);
        return;
    }
    
    println!("Java example code written to: {:?}", example_path);
    println!("You can now use the Java parser to analyze this file.");
    println!();
    println!("Example usage:");
    println!("1. Use JavaParser to parse the file");
    println!("2. Extract method definitions, class definitions, etc.");
    println!("3. Build call graph to understand method dependencies");
    println!("4. Analyze package structure and imports");
    println!();
    println!("The example includes:");
    println!("- Package declaration");
    println!("- Import statements");
    println!("- Class definitions with methods");
    println!("- Interface definition");
    println!("- Inheritance (extends)");
    println!("- Interface implementation");
    println!("- Exception handling");
    println!("- Static methods");
    println!("- Factory pattern");
    println!("- Annotations and documentation");
} 